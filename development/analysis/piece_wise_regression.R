...
# Plotting function
make_segmented_plot <- function(data, x_var, y_var, x_names, y_names, bottom=TRUE, left=TRUE) {
  ...
  if (!is.null(seg.mod)) {
    ...
    p <- p +
      ...
      geom_vline(xintercept = bp, color = "blue", linetype = "dashed")
...
for (y in y_vars) {
  ...
  left <- TRUE
  for (x in x_vars) {
    result_cur <- make_segmented_plot(final_data, x, y, x_names, y_names, bottom, left)
