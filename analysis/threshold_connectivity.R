...
make_segmented_plot <- function(data, x_var, y_var, x_names, y_names, controls=c(), bottom=TRUE, left=TRUE, color_number=1) {
  df <- data %>% dplyr::select(site, year, all_of(x_var), all_of(y_var), all_of(controls)) %>% na.omit
  names(df)[3:4] <- c("x", "y")
  
  if (length(controls) > 0) {
    formula <- as.formula(paste("y", "~", "x", "+", paste(controls, collapse = " + ")))
  } else {
    formula <- as.formula(paste("y", "~", "x"))
  }
  
  lin.mod <- lm(formula, data = df)
...
# Define variables
x_vars <- c("COH", "CBC_MSIZ_share")  # Extend this list as needed
...
controls <- c("perc3", "temperature_moyenne24h", "precipitation_somme24h", "radiation_somme24h", "alt", "latitude")
...
for (y in y_vars) {
  ...
  left <- TRUE
  for (x in x_vars) {
    result_cur <- make_segmented_plot(final_data, x, y, x_names, y_names, controls, bottom, left, color_number=row_number)
