...
plot_forest_by_category_pretty <- function(data, 
  ...
  ggplot(coef_df, aes(x = estimate, y = fct_rev(category), color = metric, shape = split)) +
    ...
    facet_grid(predictor ~ metric, scales = "free_y", space = "free_y", switch = "y") +
...
# If doing a continuous diff in diffs
...
final_data <- final_data %>% mutate(COH = COH*100) %>%
  ...
  filter(perc4 < 20) #%>% filter(COH<1) #%>% filter(COH<1.70157)

# If testing for difference between slopes
final_data <- final_data %>% mutate(dummy_COH = if_else(COH<1.70157, 0, 1))
#
x2 <- "dummy_COH"

result <- regression_table_long(final_data, y, x, x2, x_controls, col_to_stand, interaction = TRUE, standardise = TRUE, square=FALSE)
